    ...
    window.addEventListener('message', function (event) {
        ...
        if (data.action == "ui") {
            ...
            if (data.toggle) {
                ...
                setTimeout(function(){
					setCharactersList()
                    $.post('https://LOST-multicharacter/setupCharacters');
                    setTimeout(function(){
                        ...
                        $.post('https://LOST-multicharacter/removeBlur');
                    }, 2000);
                }, 2000);
            } else {
...
$('.disconnect-btn').click(function(e){
    ...
    $.post('https://LOST-multicharacter/closeUI');
    $.post('https://LOST-multicharacter/disconnectButton');
});
...
function setupCharInfo(cData) {
    if (cData == 'empty') {
        ...
        '<div class="character-info-box"><span id="info-label">التاريخ</span><span class="char-info-js">'+cData.charinfo.birthdate+'</span></div>' +
        '<div class="character-info-box"><span id="info-label">الجنس</span><span class="char-info-js">'+gender+'</span></div>' +
        '<div class="character-info-box"><span id="info-label">الجنسية</span><span class="char-info-js">'+cData.charinfo.nationality+'</span></div>' +
        '<div class="character-info-box"><span id="info-label">الوظيفة</span><span class="char-info-js">'+cData.job.label+'</span></div>' +
	    '<div class="character-info-box"><span id="info-label">المستوى</span><span class="char-info-js">' + cData.job.grade.name + '</span></div>' +
        '<div class="character-info-box"><span id="info-label">الكاش</span><span class="char-info-js">&#36; '+dollar.format(cData.money.cash)+'$</span></div>' +
        '<div class="character-info-box"><span id="info-label">البنك</span><span class="char-info-js">&#36; '+dollar.format(cData.money.bank)+'$</span></div>' +
        '<div class="character-info-box"><span id="info-label">رقم الهاتف</span><span class="char-info-js">'+cData.charinfo.phone+'</span></div>' +
        '<div class="character-info-box"><span id="info-label">رقم حسابك</span><span class="char-info-js">'+cData.charinfo.account+'</span></div>');
    }
}
...
$(document).on('click', '.character', function(e) {
    ...
    if (selectedChar === null) {
        ...
        if ((selectedChar).data('cid') == "") {
            ...
            $.post('https://LOST-multicharacter/cDataPed', JSON.stringify({
                cData: cDataPed
            }));
        } else {
            $(selectedChar).addClass("char-selected");
...
$(document).on('click', '#create', function (e) {
    ...
    $.post('https://LOST-multicharacter/createNewCharacter', JSON.stringify({
        firstname: firstname,
        lastname: lastname,
...
$(document).on('click', '#accept-delete', function(e){
    $.post('https://LOST-multicharacter/removeCharacter', JSON.stringify({
...
$(document).on('click', '#play', function(e) {
    ...
    if (selectedChar !== null) {
        if (charData !== "") {
            $.post('https://LOST-multicharacter/selectCharacter', JSON.stringify({
